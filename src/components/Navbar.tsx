...
function Navbar() {
  ...
  return (
    <AppBar position="sticky" sx={{ height: '150px', marginBottom: '50px',boxShadow: 'none', backgroundColor: 'transparent'}}>
            ...
            >
              {pages.map((page) => (
                <MenuItem key={page} onClick={handleCloseNavMenu}>
                  <Typography textAlign="center" component={Link} to="/categories" >{page}</Typography>
                </MenuItem>
              ))}
            </Menu>
          </Box>
          ...
          <Tooltip title="Open wishlist">
            ...
            >
              ...
              <Grid container item xs={12} justifyContent="space-around" alignItems="center">
                  <Typography variant='h5'>
                      Total price: {cart.items.map((item: any) => (item.price * item.quantity)).reduce((accumulator, currentValue) => accumulator + currentValue)}
                    </Typography>
                    <Button variant="outlined" sx={{ color: "#000", borderColor: "#000", '&:hover': { backgroundColor: '#000', color: '#fff', borderColor: "#000"
          } }}>
                                      Go to checkout
                                      <ShoppingCartIcon />
                                      </Button>
